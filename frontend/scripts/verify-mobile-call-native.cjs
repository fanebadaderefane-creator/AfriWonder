...
const nativeChecks = [
  ...
  ['PATCH_STREAM_NULL_GUARD Logcat', /logNativeStreamNullGuardActive|PATCH_STREAM_NULL_GUARD/],
  ['Anti-crash media_nudge différé (startMediaNudgeTimer)', /startMediaNudgeTimer/],
  ['Anti-crash bind RTCView stream null', /bindNativeRemoteStreamToRtcView[\s\S]*if \(!stream\) return/],
  ['Helpers safeGetAudioTracks', /callStreamTracks/],
  ...
  ['prepareNativeRtcViewUnmount avant scheduleStopAllMedia', /prepareNativeRtcViewUnmount[\s\S]*scheduleStopAllMedia/],
  ['Garde bind RTCView pendant teardown', /bindNativeRemoteStreamToRtcView[\s\S]*shouldBlockNativeRtcUrlUpdate/],
...
const testsOk = run('npm', [
  ...
  'test',
  '--',
  'src/call/callStreamTracks.test.ts',
