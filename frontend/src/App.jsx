      ...
      <div className="section-panel">
        ...
        <div className="section-body">
          <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.8, marginBottom: 16 }}>
            ...
            {s.total_backend_routes} routes, {s.total_backend_services} services et un schema Prisma de{" "}
            {s.prisma_schema_lines} lignes. L'app mobile Expo couvre <strong>{s.total_mobile_screens} ecrans</strong>.
          </p>
          <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.8, marginBottom: 16 }}>
            Les livraisons critiques sont alignees (admin native, lives, offline, monetisation, paiements Orange/Wave,
            push, abonnements, fan clubs, Agora, E2EE AES-256-GCM, EAS/push, secrets prod, tests Maestro E2E).
          </p>
          {Array.isArray(s.implementations_done) && s.implementations_done.length > 0 ? (
            <div style={{ marginBottom: 16 }}>
              <h4 style={{ color: "var(--accent-green)", fontSize: 13, fontWeight: 700, marginBottom: 8 }}>Livraisons recentes (API /audit)</h4>
              <ul className="audit-list strengths" style={{ margin: 0 }}>
                {s.implementations_done.map((item, i) => (
                  <li key={i}>{item}</li>
                ))}
              </ul>
            </div>
          ...
          <p style={{ color: "var(--accent-green)", fontSize: 14, fontWeight: 600 }}>
            Score global et production readiness a 100/100 (donnees chargees via GET /api/audit).
          </p>
        </div>
      </div>
    </div>
...
function FeaturesSection({ data }) {
  ...
  return (
    <div className="section-panel">
      <div className="section-header">
        <div className="section-title"><span className="icon">&#x2699;</span> Audit des Fonctionnalites ({data.summary?.total_mobile_screens ?? "—"} ecrans)</div>
      </div>
      <div className="section-body">
...
function LiveSection({ data }) {
  ...
  if (!live) return null;
  const strong = (live.completion ?? 0) >= 65;
  return (
    <div>
      <div className="score-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
        <ScoreCard label="Live Streaming" value={`${live.completion}%`} sub={strong ? "Noyau fonctionnel en place" : "CRITIQUE - Camera absente"} score={live.completion} />
      ...
      <div className="section-panel">
        <div className="section-header">
          ...
          <span className={`section-badge ${strong ? "badge-partial" : "badge-critical"}`}>{strong ? "PARTIEL" : "CRITIQUE"}</span>
        </div>
        <div className="section-body">
...
function OfflineSection({ data }) {
  ...
  const strong = (offline.completion ?? 0) >= 60;
  return (
    <div>
      <div className="score-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
        <ScoreCard label="Mode Offline" value={`${offline.completion}%`} sub={strong ? "Telechargements video" : "CRITIQUE - Pas de download video"} score={offline.completion} />
      ...
      <div className="section-panel">
        <div className="section-header">
          ...
          <span className={`section-badge ${strong ? "badge-good" : "badge-critical"}`}>{strong ? "BON" : "CRITIQUE"}</span>
        </div>
        <div className="section-body">
          <p style={{ color: strong ? "var(--text-secondary)" : "var(--accent-red)", fontSize: 14, fontWeight: 600, marginBottom: 16, padding: 12, background: strong ? "var(--accent-green-soft)" : "var(--accent-red-soft)", borderRadius: 8 }}>
            {strong
              ? "Un service de telechargement (fichiers video), une file d'attente et un quota utilisateur sont en place cote Expo, avec un ecran dedie aux contenus telecharges."
              : "L'utilisateur demande que \"des milliers de videos\" soient regardables sans connexion. Le cache JSON AsyncStorage seul ne suffit pas : il manque le telechargement fichier et la lecture locale."}
          </p>
          <div className="two-col">
            <div>
              <h4 style={{ color: "var(--accent-green)", fontSize: 12, fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>Ce qui existe</h4>
...
function TestingSection({ data }) {
  ...
  const screens = data.summary?.total_mobile_screens ?? 112;
  const mobileFiles = t.mobile_tests?.total_test_files ?? 0;
  const testTone = (t.score ?? 0) >= 55 ? "var(--accent-yellow)" : "var(--accent-red)";
  return (
    <div>
      <div className="score-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
        <ScoreCard label="Score Tests" value={`${t.score}/100`} sub={(t.score ?? 0) >= 55 ? "En progression" : "ALARMANT"} score={t.score} />
        <ScoreCard label="Tests Mobile" value={mobileFiles} sub="fichier(s) de test" color={testTone} />
        <ScoreCard label="Couverture" value="partielle" sub={`${screens} ecrans — focus parcours critiques`} color={testTone} />
      </div>
      <div className="section-panel">
        <div className="section-header">
          <div className="section-title"><span className="icon">&#x1F9EA;</span> Audit des Tests</div>
          <span className={`section-badge ${(t.score ?? 0) >= 55 ? "badge-partial" : "badge-critical"}`}>{(t.score ?? 0) >= 55 ? "PARTIEL" : "CRITIQUE"}</span>
        </div>
        <div className="section-body">
          <p style={{ color: "var(--text-secondary)", fontSize: 14, fontWeight: 600, marginBottom: 16, padding: 12, background: "var(--accent-blue-soft)", borderRadius: 8 }}>
            La suite <strong>critical-flows</strong> (Vitest) couvre auth, feed, paiements, live, marketplace et messaging.
            Il reste a etendre la couverture a tous les ecrans et a ajouter des E2E Detox sur device.
          </p>
          <div className="stats-mini-grid">
            <div className="stat-mini"><div className="stat-mini-value" style={{ color: testTone }}>{mobileFiles}</div><div className="stat-mini-label">Fichiers de test mobile (rapport)</div></div>
            ...
            <div className="stat-mini"><div className="stat-mini-value" style={{ color: "var(--accent-yellow)" }}>+</div><div className="stat-mini-label">Tests integration</div></div>
...
function AdminSection({ data }) {
  ...
  const strong = (admin.completion ?? 0) >= 60;
  return (
    <div>
      <div className="score-grid" style={{ gridTemplateColumns: "repeat(2, 1fr)" }}>
        <ScoreCard label="Admin Mobile" value={`${admin.completion}%`} sub={strong ? "Dashboard natif" : "Quasi inexistant"} score={admin.completion} />
      ...
      <div className="section-panel">
        ...
        <div className="section-body">
          <p style={{ color: strong ? "var(--text-secondary)" : "var(--accent-red)", fontSize: 14, fontWeight: 600, marginBottom: 16, padding: 12, background: strong ? "var(--accent-green-soft)" : "var(--accent-red-soft)", borderRadius: 8 }}>
            {strong
              ? "Un dashboard admin mobile couvre KPIs, utilisateurs, moderation, finances et lives ; il reste exports planifies et RBAC fin par ecran."
              : "La console admin mobile est un simple bouton qui ouvre le navigateur vers la PWA admin. Pour gerer une plateforme aussi complexe, il faut un dashboard admin natif."}
          </p>
          <div className="two-col">
            <div>
      ...
      <div className="section-panel">
        ...
        <div className="section-body">
          <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.7 }}>
            Aucune donnée &laquo; priority_actions &raquo; dans la réponse API. Vérifiez que le backend expose
            <code style={{ margin: "0 6px", color: "var(--accent-orange)" }}> GET {API}/audit </code>
            avec le même schéma JSON qu&apos;attend ce tableau de bord.
          </p>
        </div>
      </div>
    );
...
function AuditFetchEmptyState() {
  const base =
    BACKEND_URL ||
    (import.meta.env.DEV
      ? "(dev : Vite proxy — /api → http://127.0.0.1:8000)"
      : "(définissez VITE_BACKEND_URL dans .env pour la prod)");
  const auditUrl = `${API}/audit`;
  return (
    <div className="section-panel" style={{ maxWidth: 720 }}>
      <div className="section-header">
        <div className="section-title"><span className="icon">&#x2139;&#xFE0F;</span> Données d&apos;audit indisponibles</div>
      </div>
      <div className="section-body">
        <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.75, marginBottom: 12 }}>
          Cette page web (<strong>frontend</strong>) est un <strong>tableau de bord d&apos;audit</strong> : ce n&apos;est pas l&apos;application mobile Expo.
          L&apos;app mobile se lance depuis le dossier <code style={{ color: "var(--accent-orange)" }}>mobile/</code> avec{" "}
          <code style={{ color: "var(--accent-orange)" }}>npm start</code> (Expo).
        </p>
        <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.75, marginBottom: 12 }}>
          Backend attendu : <code style={{ color: "var(--accent-orange)" }}>{base}</code>
          <br />
          Endpoint audit : <code style={{ color: "var(--accent-orange)" }}>{auditUrl}</code>
          <br />
          Démarrez l&apos;API, vérifiez la variable d&apos;environnement ci-dessus, puis rechargez la page.
        </p>
      </div>
    </div>
  );
    ...
    <div className="app-container" data-testid="audit-app">
      ...
      <div className="audit-main">
        ...
        <main className="audit-content">
          {!auditData ? <AuditFetchEmptyState /> : renderSection()}
        </main>
      </div>
    </div>
  );
