  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        const d = getDemoProvidersForCategory('childcare');
        if (d.length > 0) {
          setProviders(d);
          setShowDemo(true);
        } else {
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {loading ? (
        ...
        <View style={styles.centerBox}>
          ...
          <Text style={styles.errorTitle}>Prestataires indisponibles</Text>
        ...
        <View style={styles.centerBox}>
          ...
          <Text style={styles.emptyText}>
            Aucun prestataire de garde d'enfants disponible pour l'instant.
          </Text>
        </View>
      ) : (
        <ScrollView
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <View style={styles.nameRow}>
                  <Text style={styles.cardName} numberOfLines={1}>{p.display_name ?? p.full_name ?? 'Prestataire'}</Text>
                ...
                <View style={styles.cardMeta}>
                  ...
                  {p.total_jobs ? (
                    <Text style={styles.metaText}>{p.total_jobs} interventions</Text>
                  ) : null}
                  {p.city ? (
                ...
                {typeof p.base_price === 'number' ? (
                  <Text style={styles.priceText}>
                    À partir de {p.base_price.toLocaleString()} {p.currency ?? 'FCFA'}
                  </Text>
                ) : null}
              </View>
            </TouchableOpacity>
