...
export default function SearchScreen() {
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      const raw = q.trim();
      const tagOnly = raw.replace(/^#+/, '').trim();
      const params: Record<string, string> = { q: raw, type: typeParam };
      if (tab === 'Hashtags' && tagOnly) {
        params.q = `#${tagOnly}`;
        params.hashtag = tagOnly;
      }
  ...
  const renderSound = ({ item }: any) => (
    <TouchableOpacity
      ...
      onPress={() => {
        const sid = String(item.sample_video_id || '').trim();
        if (sid) router.push({ pathname: '/watch/[id]', params: { id: sid } });
      }}
      activeOpacity={0.85}
      accessibilityLabel={`Son ${item.title}`}
    >
      <View style={styles.soundIconWrap}>
        <Ionicons name="musical-notes" size={22} color={Colors.primary} />
      </View>
      <View style={{ flex: 1 }}>
        <Text style={styles.videoTitle} numberOfLines={2}>{item.title || 'Son'}</Text>
        <Text style={styles.videoMeta}>{item.video_count ?? 0} vidéos</Text>
      </View>
      ...
      {loading ? <ActivityIndicator size="large" color={Colors.primary} style={{ marginTop: 40 }} /> : !hasResults ? (
        ...
        <FlatList
          data={[
            ...
            ...(results.sounds || []).map((s: any, i: number) => ({ ...s, _type: 'sound', id: s.title + i })),
...
const styles = StyleSheet.create({
  ...
  tabsScroll: { marginBottom: Spacing.md, maxHeight: 44 },
  tabsRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: Spacing.lg, gap: 4 },
  ...
  userRowMain: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: Spacing.md },
  soundIconWrap: {
    width: 100,
    height: 130,
