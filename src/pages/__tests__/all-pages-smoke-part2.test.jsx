/**
 * Smoke render pages 2/4 (Crowdfunding -> Microcredit).
 */
...
vi.mock('@/api/expressClient', () => {
  ...
  const api = { get: vi.fn().mockResolvedValue({ data: [] }), post: vi.fn().mockResolvedValue({ data: {} }), put: vi.fn().mockResolvedValue({ data: {} }), patch: vi.fn().mockResolvedValue({ data: {} }), delete: vi.fn().mockResolvedValue({}), auth: { me: vi.fn().mockResolvedValue({ id: '1', email: 'test@test.com' }) }, e2ee: { registerDevice: vi.fn().mockResolvedValue({}), uploadPrekeys: vi.fn().mockResolvedValue({}), getPrekeyHealth: vi.fn().mockResolvedValue({ healthy: true, availablePrekeys: 10 }), rotateSignedPrekey: vi.fn().mockResolvedValue({}), getBundle: vi.fn().mockResolvedValue({}), consumePrekey: vi.fn().mockResolvedValue({}), syncEnvelopes: vi.fn().mockResolvedValue({ envelopes: [] }), getUserPublicDevices: vi.fn().mockResolvedValue([]) }, videos: { list: vi.fn().mockResolvedValue({ videos: [] }), getComments: vi.fn().mockResolvedValue({ comments: [] }), getById: vi.fn().mockResolvedValue({ id: '1', video_likes: [], video_saves: [] }) }, users: { getFollowing: vi.fn().mockResolvedValue({ following: [] }), getProfile: vi.fn().mockResolvedValue({}), getLikedVideos: vi.fn().mockResolvedValue({ videos: [] }) }, saves: { list: vi.fn().mockResolvedValue({ videos: [] }) }, payments: { getWallet: vi.fn().mockResolvedValue({ balance: 0 }) }, orders: { getById: vi.fn().mockResolvedValue({ id: '1', status: 'pending', items: [], shipping: {} }) }, refunds: { listMy: vi.fn().mockResolvedValue({ refunds: [] }) }, providers: { list: vi.fn().mockResolvedValue({ providers: [] }) }, support: { listTickets: vi.fn().mockResolvedValue({ tickets: [] }), getTicket: vi.fn().mockResolvedValue(null), createTicket: vi.fn().mockResolvedValue({}), addMessage: vi.fn().mockResolvedValue({}) }, entities: new Proxy({}, { get: () => em }) };
...
const SLICE = entries.slice(32, 64);

describe('Smoke render pages (partie 2/4)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
