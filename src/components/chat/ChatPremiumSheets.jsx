...
export function ChatAttachmentSheet({
  ...
  onToggleEphemeral,
  onAudioHint,
  /** Ouvre le flux sondage (1-1). Si absent, onPollSoon est utilisé. */
  onPoll,
  onPollSoon,
  onEvent,
  onEventSoon,
  ...
  const items = useMemo(
    () => [
      ...
      {
        key: 'poll',
        label: labels.attachPoll,
        icon: BarChart3,
        color: 'bg-yellow-500',
        onClick: onPoll ?? onPollSoon,
      },
      {
        key: 'event',
        label: labels.attachEvent,
        icon: CalendarDays,
        color: 'bg-fuchsia-600',
        onClick: onEvent ?? onEventSoon,
      },
