...
import { Users, Ban, Filter } from 'lucide-react';
...
export default function UsersPanel() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.admin.getUsers({ page, limit: PAGE_SIZE }),
  });
    ...
    <Card className="p-6 bg-white/10 backdrop-blur border-white/20 text-white">
      ...
      <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/10">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" className="border-white/20 text-white" disabled={pagination.page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Precedent</Button>
          <Button size="sm" variant="outline" className="border-white/20 text-white" disabled={pagination.page >= (pagination.totalPages || 1)} onClick={() => setPage((p) => p + 1)}>Suivant</Button>
        </div>
      </div>
    </Card>
  );
